...
public class SOManager
{
  ...
  static
  {
    ...
    if (osName.startsWith("mac os x")) so=SOManager.MAC_OS_X;
    else if (osName.startsWith("windows")) so=SOManager.WINDOWS;
    else if (osName.startsWith("linux")) so=SOManager.LINUX;
    else so=SOManager.UNKNOWN;
  }
   ...
   */
  public static int getSO() {return so;}
  ...
  public static int checkMouseButton(MouseEvent e)
  {
    ...
    switch (SOManager.getSO())
    {
      case SOManager.MAC_OS_X:
        ...
        {
          if (e.isControlDown()) return MouseEvent.BUTTON3;
          else return MouseEvent.BUTTON1;
        }
        else return button;
      default:
        return button;
