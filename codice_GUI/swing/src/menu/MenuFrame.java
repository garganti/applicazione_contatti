...
public class MenuFrame extends JFrame implements ActionListener, MouseListener
{
  ...
  private String[] icon={"ball-red.png","ball-green.png","ball-blue.png","ball-yellow.png","ball-orange.png","ball-pink.png"};
  ...
  {
    JOptionPane.showMessageDialog(this,"La finestra delle preferenze","Preferenze",JOptionPane.INFORMATION_MESSAGE);
  }

  public void quit()
  {
    int option=JOptionPane.showConfirmDialog(this,"Sei Sicuro di voler uscire?","Esci?",JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE);
  ...
  public void actionPerformed(ActionEvent e)
  {
    ...
    for (int i=0;i<colori.length;i++)
    {
      if (source==sfondo[i]) pannelloFiore.setSfondo(colori[i]);
      else if (source==petalo[i]) pannelloFiore.setPetali(colori[i]);
      else if (source==cuore[i]) pannelloFiore.setCuore(colori[i]);
  ...
  public static void main(String[] args)
  {
    ...
    //Verifico se il sistema operativo Host e' MacOSX ed attivo lo stile dei menu di MacOSX
    //if (SOManager.getSO()==SOManager.MAC_OS_X) OSXAdapter.registerMacOSXApplication(frame,true);
...
class MenuPanel extends JPanel
{
  ...
  public int getType(Point p)
  {
    ...
    if (cuoreShape.contains(p)) return this.CUORE;
    else if (petaliShape.contains(p)) return this.PETALO;
    else return this.SFONDO;
  }
  ...
  public void paintComponent(Graphics g)
  {
    ...
    for (int i=0;i<8;i++)
    {
      ...
      petaliShape.add(new Area(new Ellipse2D.Double(d.width/2+raggioFiore*Math.cos(angolo)-raggioPetalo,
                                                    d.height/2+raggioFiore*Math.sin(angolo)-raggioPetalo,
                                                    2*raggioPetalo,2*raggioPetalo)));
    }
    g2.fill(petaliShape);

    g.setColor(cuore);
    //Creo la forma del cuore
    cuoreShape=new Ellipse2D.Double(d.width/2-raggioFiore,d.height/2-raggioFiore,2*raggioFiore,2*raggioFiore);
    g2.fill(cuoreShape);
  }
