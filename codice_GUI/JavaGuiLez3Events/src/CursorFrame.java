...
class CursorPanel extends JPanel implements MouseMotionListener
{
  ...
  {
    Point p=e.getPoint();
    if (xRect<=p.x&&p.x<xRect+wRect&&
        yRect<=p.y&&p.y<yRect+hRect) setCursor(new Cursor(Cursor.CROSSHAIR_CURSOR));
    else if (xRect2<=p.x&&p.x<xRect2+wRect2&&
             yRect2<=p.y&&p.y<yRect2+hRect2) setCursor(new Cursor(Cursor.WAIT_CURSOR));
    else if (Math.sqrt((p.x-xOval)*(p.x-xOval)+(p.y-yOval)*(p.y-yOval))<rOval) setCursor(new Cursor(Cursor.HAND_CURSOR));
    else if (Math.sqrt((p.x-xOval2)*(p.x-xOval2)+(p.y-yOval2)*(p.y-yOval2))<rOval2) setCursor(custom);
    else if (getWidth()-10<p.x&&getHeight()-10<p.y) setCursor(new Cursor(Cursor.SE_RESIZE_CURSOR));
    else if (getWidth()-10<p.x) setCursor(new Cursor(Cursor.E_RESIZE_CURSOR));
    else if (getHeight()-10<p.y) setCursor(new Cursor(Cursor.S_RESIZE_CURSOR));
